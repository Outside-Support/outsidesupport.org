...
6. Run `testAlertEmailToJim` once and confirm `jim@outsidesupport.org` receives the test alert.
7. Run `testAlertEmail` once and confirm `team@outsidesupport.org` receives the test alert.
8. Deploy as a web app:
   - Execute as: `Me`
   - Who has access: `Anyone`
9. Copy the deployed web app URL into `contact-config.js` as `endpoint`.
...
If `testAlertEmailToJim` works but `testAlertEmail` does not arrive, Apps Script email sending is working and `team@outsidesupport.org` needs to be checked as a mailbox, alias, or Google Group.
